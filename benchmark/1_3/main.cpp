...
namespace {
  ...
  void CustomArguments(benchmark::internal::Benchmark* b) {
    // 99 unique numbers in [100,1000000] range
    ...
    for (size_t i = 0; i < args.size(); i += 3) {
      b->Args({args[i], args[i + 1], args[i + 2]});
    }
  }
}   // namespace
...
auto main(int32_t argc, char** argv) -> int32_t {
    ...
    [](benchmark::State& state) {
      for ([[maybe_unused]] auto _ : state) {
        benchmark::DoNotOptimize(
          Problem_1_3::StandardLCM({static_cast<uintmax_t>(state.range(0)),
                                    static_cast<uintmax_t>(state.range(1)),
                                    static_cast<uintmax_t>(state.range(2))}));
      }
    })
  ...
  benchmark::RegisterBenchmark(
    ...
    [](benchmark::State& state) {
      for ([[maybe_unused]] auto _ : state) {
        benchmark::DoNotOptimize(
          Problem_1_3::BinaryLCM({static_cast<uintmax_t>(state.range(0)),
                                  static_cast<uintmax_t>(state.range(1)),
                                  static_cast<uintmax_t>(state.range(2))}));
      }
    })
